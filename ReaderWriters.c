...
int main(int argc, char **argv) {
    ...
    pthread_t *rThreads = malloc(numRThreads * sizeof(pthread_t));
    pthread_t *wThreads = malloc(numWThreads * sizeof(pthread_t));

    int *rIDs = malloc(numRThreads * sizeof(int));
    int *wIDs = malloc(numWThreads * sizeof(int));

    for (int i = 0; i < numRThreads; i++) {
        rIDs[i] = i;
        pthread_create(&rThreads[i], NULL, readers, &rIDs[i]);
    }
    for (int i = 0; i < numWThreads; i++) {
        wIDs[i] = i;
        pthread_create(&wThreads[i], NULL, writers, &wIDs[i]);
    }
